...
public class Main extends Application {
    ...
    public void start(Stage primaryStage) {
       ...
       logger.info("check last update complete->{}",checkCompletLastUpdate);
        ...
        if(need_update==true) 
        {
            ...
            if(updater.last_result==Updater.RESULT_NEED_REBOOT ){
                ExitDialog dlg = new ExitDialog("Необходимо перегазурить компьютер.",false);
                dlg.showAndWait();
                Platform.exit();
                return;
            }else if(updater.last_result==Updater.RESULT_NEED_RELOAD){
                 Platform.exit();
            }
        ...
        try {
            int runClientApp = runClientApp();
            if(runClientApp!=Updater.RESULT_NEED_RELOAD){
                String text = "Компьютер сейчас будет выключен!";
                ExitDialog dlg = new ExitDialog(text,false);
                dlg.showAndWait();
            }
           
           updater.last_result = runClientApp;
        }
        catch(Throwable tw){
    ...
    public static void main(String[] args)  {
            ...
            try {
                launch(args);
                logger.debug("LAUNCHER exit with code: {}",updater.last_result);
